...
class SetBaremetalRunbook(command.Command):
    ...
    def take_action(self, parsed_args):
        ...
        try:
            ...
            utils.check_api_version_support(
                current_version, "1.112", "Runbook description field")
        except exc.UnsupportedVersion:
            supports_new_fields = False
...
class RemoveTraitBaremetalRunbook(command.Command):
    ...
    def take_action(self, parsed_args):
        ...
        if parsed_args.remove_all:
            ...
            print(_('Removed all traits from runbook %s') % parsed_args.runbook)
        else:
            for trait in parsed_args.traits:
                try:
                    baremetal_client.runbook.remove_trait(parsed_args.runbook, trait)
